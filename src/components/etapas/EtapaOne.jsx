...
import styles from "./EtapaOne.module.css";
export const EtapaOne = ({ numero, children, onNext, next, dataEtapa }) => {
  ...
  useEffect(() => {
    if (selectedEstado) {
      axios
        .get(
          `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${selectedEstado.id}/distritos?orderBy=nome`
        )
        .then((res) => {
          const response = res.data;
          setCidades(response);
  ...
  return (
    <div className={[styles.container_etapa_type]}>
      <h2>
        <i className="ri-signpost-line"></i> 0{numero}|03
      </h2>
      <div className={styles.box_avatar_acomp}>
        <div className={styles.avatar_img_acomp}>
          {imgAvatar ? (
        ...
        <div className={styles.avatar_input_acomp}>
          <input
            required
            type="file"
            accept="image/png, image/jpg, image/jpeg, image/webp"
            name=""
            id="avatar_profile_acomp"
            onChange={handleFileChange}
          />
          <label htmlFor="avatar_profile_acomp">
            <span>
              <i className="ri-upload-cloud-fill"></i>Enviar Foto
            </span>
          </label>
        </div>
      </div>
      <div className={styles.modal_name_lastname}>
        <input
          type="text"
          name=""
          id=""
      ...
      <div className={styles.modal_about}>
        ...
        ></textarea>
        <span
          className={countNumber.length === 150 ? styles.limitCaracteres : ""}
        >
          {countNumber === "" || countNumber.length === 0
            ? 0
            : countNumber.length}
          /150
        </span>
      </div>
      <div className={styles.modal_genero_estado_acomp}>
        <div className={styles.modal_date_acomp}>
        ...
        <div className={styles.modal_estado_cidade_acomp}>
          <div className={styles.modal_estado_acomp}>
            <select
              name=""
              id=""
            ...
            >
              ...
              {estados.map((item, index) => (
                <option key={index} title={item.nome} value={item.nome}>
                  {item.nome}
                </option>
              ))}
            </select>
            {selectedEstado && selectedEstado.sigla !== undefined ? (
              <span>{selectedEstado.sigla}</span>
            ) : (
              <span>Sigla</span>
            )}
          </div>
          {selectedEstado && (
            <div className={styles.modal_cidade_acomp}>
              <select
                name=""
                id=""
